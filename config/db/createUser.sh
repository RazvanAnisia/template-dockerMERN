...
docker exec -it mysql-db mysql -u root -p$DB_ROOT_PASSWORD && CREATE USER `$DB_USER`@'localhost' IDENTIFIED BY $DB_PASSWORD && GRANT ALL PRIVILEGES ON * . * TO `$DB_USER`@'localhost';